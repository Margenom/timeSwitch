...
;planed
	((select) (args-check 1) (part-check) (proc-check) (let ((busy (list-ref Busy (string->number (list-ref CLI_ARGS 2)))))
	...
	)((timer) (part-check) (proc-check) (need-args? 1 1 -1) (let ((busy (list-ref Busy (string->number (list-ref CLI_ARGS 2)))))
		(dlg-app-begin DoneLogFile busy)
		(if (param-or-val "m" #f)  (notify "Select busy" (busy-name busy)))
	...
	)((check) (args-check 1) (let ((musk (make-list 5 -1)) (time (busy-clock (clock-seconds))))
		(case (string->symbol (list-ref CLI_ARGS 2))
			((day) ;(map (lambda(k) (print-ranged (busy-hour k) (busy-hour busy-limits) (busy-id k)))
			 (let ((busyes (filter (lambda(b) (busy-parttime? b (list-patch time '(0 . -1) '(1 . -1)))) Busy)))
							(print-ranges	(map busy-hour busyes) (busy-hour busy-limits) (map busy-id busyes))))  
			;((week) (map print (filter (lambda(b) (apply and (map (lambda(k (busy-parttime? b (list-patch musk (cons 3 (busy-)))) Busy)))
			((mouth) (map print (filter (lambda(b) (busy-parttime? b (list-patch musk (cons 3 (busy-month time))))) Busy)))
			 (else (map print (filter (lambda(b) (busy-parttime? b (list-patch time '(0 . -1) '(1 . -1)))) Busy)))))
...
;unplaned
	...
	)((tell) (args-check #f) (dlg-app-tail DoneLogFile
		(if (param-or-val "g" #f) (gui-input-mesg "Tell me what you do") (apply string-join " " (cddr CLI_ARGS))))
		...
		(let*((start (clock-seconds))
				...
				(stop (clock-seconds))
				(clistring (if (null? (cddr CLI_ARGS)) inpstring (apply string-join " " cldata))))
