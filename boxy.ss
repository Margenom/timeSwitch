...
;planed
	((select) (args-check 1) (part-check) (proc-check) (let ((busy (list-ref Busy (string->number (list-ref CLI_ARGS 2)))))
		...
		(if (or (null? (cddr CLI_ARGS)) (apply and (map (lambda(r) (list? (rex-match? r (busy-name b)))) cldata))) (busy-print b))) Busy)
		...
		(case (string->symbol ((lambda(a) (if (null? a) "undef" (car a))) (cddr CLI_ARGS)))
			...
			(else ;(map (lambda(k) (print-ranged (busy-hour k) (busy-hour busy-limits) (busy-id k)))
