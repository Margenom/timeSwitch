...
(let rec((ost parsed) (out '()) (waits '()) (head #f)) 
	(if (null? ost) (reverse (if (and allow_processed head) (cons (list 'proc head (reverse waits)) out) out)) ; like processing
		(if (car ost) (let*( (mark (string-ref (caar ost) 0)) (unt (dlg-type-remove (car ost))))
		(case mark
			; wait
			((#\tab) (rec (cdr ost)
